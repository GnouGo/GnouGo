...
try {
  ...
  const totals = page.getByRole('region', { name: 'Estimated costs for retained history' })
  assert.match(await totals.innerText(), /EUR.*0[.,]007136/s)
  assert.match(await totals.innerText(), /USD.*0[.,]002592/s)
  const initialTotals = await totals.innerText()
  ...
  assert.equal(await totals.innerText(), initialTotals, 'Filtering must not change retained-history totals')
  ...
  const cost = page.getByRole('region', { name: 'Request cost' })
  assert.match(await cost.innerText(), /EUR.*0[.,]002272/s)
  assert.match(await cost.innerText(), /300/)
  assert.match(await totals.innerText(), /EUR.*0[.,]009408/s)
  ...
  assert.doesNotMatch(await totals.innerText(), /EUR|USD/)
  assert.deepEqual(errors, [])
  console.log('PASS: live incremental output, native payloads, filters, setup, pause/resume, reconnect, mobile, clear, precise costs and retained-history currency totals; no browser errors.')
} finally {
  await browser.close()
}
