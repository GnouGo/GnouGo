...
public sealed class BundledBrowserMcpPublishTests
{
    ...
    public void MainWorkflow_PassesPackagePublishingSecrets()
    {
        ...
        Assert.Contains("actions: read", yaml);
        Assert.Contains("pull-requests: write", yaml);
    ...
    public void VersionWorkflow_CreatesReleaseTagAndChangelogOnlyForExplicitReleaseMarker()
    {
        ...
        Assert.Contains("Generate changelog update PR", yaml);
        Assert.Contains("peter-evans/create-pull-request@v6", yaml);
        Assert.Contains("branch: \"release/changelog/${{ steps.tag.outputs.version_tag }}\"", yaml);
        ...
        Assert.DoesNotContain("git push --force-with-lease origin \"HEAD:refs/heads/main\"", yaml);
