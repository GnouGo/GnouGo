...
public sealed partial class LiveIntentAgentGenerationTests
{
    ...
    private sealed class ComparisonAttempt(string strategy, int index, string cohort)
    {
        ...
        public int InputTokenCeiling { get; init; } = 12_000;
        ...
        public JsonObject ToJson() => new()
        {
            ...
            ["failureType"] = FailureType, ["failedPhase"] = Outcome == "passed" ? null : Outcome == "blocked_preparation" ? "shared_preparation" : Outcome == "cleanup_failed" ? "cleanup" : ExecutionStarted ? "execution" : _snapshot is null ? "preflight" : PlanningPhase.Resolve(_snapshot),
            ...
            ["activeMilliseconds"] = ActiveMilliseconds, ["withinTarget"] = Outcome == "passed" && PrefixActiveMilliseconds + ActiveMilliseconds <= 300_000,
            ["repairCalls"] = _snapshot is null ? 0 : PlanningConstructionStrategies.IsWholeWorkflow(Strategy)
            ...
            ["sourceCalls"] = PlanningConstructionStrategies.IsWholeWorkflow(Strategy) && _snapshot is not null
                ? JsonValue.Create(_snapshot.SourceCandidates.Sum(c => c.Calls)) : null,
            ["inputTokenCeiling"] = _snapshot?.Request.Generation.MaxInputTokensPerUnit ?? InputTokenCeiling,
            ...
            ["constructionUnits"] = new JsonArray((_snapshot?.SourceCandidates ?? []).Select(c => (JsonNode)new JsonObject {
                ["workflowKey"] = c.WorkflowKey, ["format"] = c.Format, ["status"] = c.Status, ["calls"] = c.Calls,
                ["repairs"] = Math.Max(0, c.Calls - 1), ["estimatedInputTokens"] = c.EstimatedInputTokens,
                ["inputTokenLimit"] = c.InputTokenLimit,
                ["diagnosticCodes"] = new JsonArray(c.Diagnostics.Select(d => (JsonNode?)JsonValue.Create(d.Code)).ToArray()) }).ToArray()),
