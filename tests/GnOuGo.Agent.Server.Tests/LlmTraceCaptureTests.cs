...
public sealed class LlmTraceCaptureTests : BunitContext
{
    ...
    public async Task HistoricalJournalReadsAreOwnedReadOnlyAndExcludeRawProviderData(bool workflow)
    {
        ...
        if (workflow) await fixture.Records.UpsertAsync("flow-planning-sessions-v9", "tenant", "session", JsonSerializer.Serialize(state, PlanningJsonContext.Default.PlanningSession), "test", Ct);
        ...
        var record = await fixture.Records.UpsertAsync(prefix + "-planning-model-requests-v9", "tenant", key, JsonSerializer.Serialize(request, PlanningJsonContext.Default.LLMRequest), "test", Ct);
        var store = Store(fixture);
        ...
        Assert.Equal(record, await fixture.Records.GetAsync(prefix + "-planning-model-requests-v9", "tenant", key, "test", Ct));
    ...
    public async Task JournalBackedCaptureReusesReservationAndReceiptWithoutDuplicatingContent()
    {
        await using var fixture = await PlanningPersistenceTests.StoreFixture.CreateAsync();
        var state = new PlanningSession { Request = new() { SessionId = "session", TenantId = "tenant" } };
        await fixture.Records.UpsertAsync("flow-planning-sessions-v9", "tenant", "session", JsonSerializer.Serialize(state, PlanningJsonContext.Default.PlanningSession), "test", Ct);
        var request = new LLMRequest { ClientRequestId = "session:1:hash", Prompt = "JOURNALED_PRIVATE_INPUT" };
        var reservation = await fixture.Records.UpsertAsync("flow-planning-model-requests-v9", "tenant", request.ClientRequestId, JsonSerializer.Serialize(request, PlanningJsonContext.Default.LLMRequest), "test", Ct);
        var harness = SmartFlowTestFactory.CreateTelemetryHarness(); using var telemetry = harness.Telemetry;
        var local = new LocalTraceDebugStore(new TestOptionsMonitor<OpenTelemetrySettings>(new() { TenantId = "tenant" }));
        var store = Store(fixture);
        var capture = new LlmTraceCapture(telemetry, store, local, NullLogger<LlmTraceCapture>.Instance);
        ...
        await fixture.Records.UpsertAsync("flow-planning-model-receipts-v9", "tenant", request.ClientRequestId, JsonSerializer.Serialize(response, PlanningJsonContext.Default.LLMResponse), "test", Ct);
        ...
        Assert.Equal(reservation, await fixture.Records.GetAsync("flow-planning-model-requests-v9", "tenant", request.ClientRequestId, "test", Ct));
    }
    ...
    public async Task RetentionDeletesOnlyExpiredDiagnosticRecordsForTheCurrentTenant()
    {
        ...
        records.Values.Add(new("flow-planning-model-requests-v9", "tenant", "request", "{}", DateTimeOffset.UtcNow.AddDays(-8), DateTimeOffset.UtcNow));
        var store = new LlmTraceContentStore(records, new TestOptionsMonitor<TraceDebugSettings>(new()), Options.Create(new OpenTelemetrySettings { TenantId = "tenant" }), fixture.Store);
        Assert.Null(await store.LoadAsync("expired", "trace", "span", null, Ct));
        await store.PurgeAsync(Ct);
        Assert.Equal(3, records.Values.Count);
        Assert.Contains(records.Values, r => r.Collection == "flow-planning-model-requests-v9");
