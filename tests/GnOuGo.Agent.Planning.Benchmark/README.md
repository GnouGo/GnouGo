...
## Occurrence-boundary MIXED gate

`schema5-occurrence-boundaries-mixed-diagnostics-1` authorizes one fresh MIXED case on the same frozen `cfc058f` production. It reads the accepted LOCAL report from `schema5-occurrence-boundaries-diagnostics-1` as a prerequisite; no historical runtime answers or receipts are imported. Effective typed policy settings, scenarios, declarations and production binaries must match the comparison manifest. LOCAL, Stage 1 and replacement starts are refused.

Before dispatch, run the offline harness tests and `diagnose-runtime-admission selfcheck`, reference checks and published persistence smokes, then `diagnose-runtime-admission freeze cfc058fd52183854d09b2540b9f57d1e337797f9`. The freeze performs one exchange-rate prerequisite check without injecting a rate into planning. The only authorized live command is `diagnose-runtime-admission run-mixed`, once. A case with any checkpoint, report, budget or reservation cannot start again; use `report` or original-schema read-only audit after interruption.

Acceptance requires exactly one proven external read, one local result realization and an evidence-supported read-to-local dependency. Existing bounded dependency questions are permitted and separately counted. After atomic admission, detached restart must preserve the whole snapshot without transport or checkpoint writes. The existing relationship path then reuses the admission data proof; its schemas cannot offer `data`. All phases share the unchanged sixteen-reservation budget. Stop at completion or the first blocker, without repairs to production or advancement to Stage 1.

This identity has now consumed its single MIXED start. It stopped on a verified realization-coverage blocker after ten schema-valid receipts; no admission/dependency proof committed. Do not repeat its live command. Use `diagnose-runtime-admission report` or `audit-runtime-admission schema5-occurrence-boundaries-mixed-diagnostics-1:mixed` for read-only evidence. See the [MIXED report](../../docs/planner-occurrence-boundaries-mixed-1.md). The recommended next gate is architecture review, not Stage 1.
