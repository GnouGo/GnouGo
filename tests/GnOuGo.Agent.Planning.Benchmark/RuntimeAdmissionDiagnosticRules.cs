...
internal static class RuntimeAdmissionDiagnosticRules
{
        ...
        IReadOnlyList<string> inputs, string output, int identityDecisions, int dependencyDecisions = 0)
    {
        ...
        Require(operations.Count == (name == "local" ? 1 : 2) && operations.All(o => o.Required && o.OperationAdmission is { Version: 18, Dependencies.Version: 1 }) &&
        ...
        Require(operations.All(o => o.OperationAdmission!.ExecutionContributions.Count > 0 &&
            o.OperationAdmission.ExecutionContributions.All(p => p.Version == 8 && !string.IsNullOrEmpty(p.ProofFingerprint)) &&
    ...
    internal static void RequirePositiveSupports(PlanningObligation operation)
    {
        ...
        bool Valid(PlanningOperationAssignment assignment)
        {
            ...
            var matches = admission.ExecutionContributions.Where(p => p.Version == 8 &&
    ...
    {
        var proof = operation.OperationAdmission!;
        var qualified = proof.ExecutionContributions.Where(p => p.Version == 8).SelectMany(p => p.Contributions).ToArray();
