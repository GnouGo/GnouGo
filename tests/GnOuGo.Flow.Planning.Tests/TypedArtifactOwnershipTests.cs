...
public sealed class TypedArtifactOwnershipTests
{
    ...
    public async Task LocalOperationOwnershipAllowsValidatedNativeControlFlow()
    {
        ...
        preparation.RuntimeState["capabilities"]!.AsArray().Add(new JsonObject
        {
            ["id"] = "local",
            ["description"] = "Local condition",
            ["required"] = true,
            ["resolution"] = "local",
            ["requestBindings"] = new JsonArray(),
            ["operationId"] = "local",
            ["executionKind"] = "local_processing",
            ["externalEffectKind"] = "none"
        });
        ...
        Assert.Empty(await runtime.ValidateAsync(new(yaml, request, preparation, bindings), Ct));
    }
    ...
    public async Task SharedConsentRetainsDistinctOwnersOfIdenticalPhysicalCalls(string server, string method)
    {
        ...
        Assert.Contains(await runtime.ValidateAsync(new(yaml, request, preparation, []), Ct), d => d.Code == "ARTIFACT_OWNERSHIP_INVALID");
        Assert.Empty(await runtime.ValidateAsync(new(yaml, request, preparation, bindings), Ct));
    ...
    public async Task OwnershipCannotHideUnapprovedOrUnboundCalls(string variation)
    {
        ...
        var findings = await runtime.ValidateAsync(new(yaml, request, preparation, bindings), Ct);
    ...
    private static async Task<(WorkflowPlanningRuntime, PlanningRequest, PlanningPreparation, InMemoryMcpClientFactory, List<JsonNode?>)> Prepare(string server, string method)
    {
        ...
        {
            TenantId = "tenant",
            Prompt = "Perform both actions only after confirmation.",
            Options = new()
            {
                ["generator"] = new JsonObject { ["model"] = "fake" },
                ["capability_preflight"] = new JsonObject
                {
                    ["requirements"] = new JsonArray(new[] { "first", "second" }.Select(id => (JsonNode)new JsonObject
        ...
        resolved.Add(new JsonObject
        {
            ...
            ["catalogId"] = "human_catalog",
            ["executionKind"] = "human_interaction",
            ["externalEffectKind"] = "none"
        });
