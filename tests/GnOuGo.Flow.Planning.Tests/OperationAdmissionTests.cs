...
public sealed class OperationAdmissionTests
{
    ...
    public async Task LocalRuntimeEvidenceResolvesWithoutAnOperationOrCapabilityRequest(string text)
    {
        ...
        await ResolveGrounded(state, runtime, Ct);
    ...
    public async Task IncompatibleLifecycleProofFailsClosed(string kind, string? action)
    {
        ...
        await Assert.ThrowsAsync<WorkflowRuntimeException>(() => ResolveGrounded(state, NoModel(), Ct));
    ...
    public async Task CapturedDeclarationFixtureKeepsPortsAndAddsOnlyTheLocalRule()
    {
        ...
        await ResolveGrounded(restarted, NoModel(), Ct);
    ...
    {
        var state = State("Transform a record."); Add(state, 0); await ResolveGrounded(state, NoModel(), Ct);
    ...
    public async Task RevisionRetiresRecognizedActionAndRestartCannotResurrectIt()
    {
        ...
        await ResolveGrounded(state, runtime, Ct); Assert.Empty(state.Obligations);
        var calls = runtime.Requests.Count; await ResolveGrounded(PlanningContext.Clone(state), runtime, Ct); Assert.Equal(calls, runtime.Requests.Count);
    }
    ...
    public async Task CancellationCannotCommitPartialAuthority()
    {
        ...
        await Assert.ThrowsAsync<OperationCanceledException>(() => ResolveGrounded(state, NoModel(), cancellation.Token)); Assert.Empty(state.Obligations);
    }
    ...
    public void ResourceResponseRequiresOwnershipAndNeverAcceptsPlanningExecutionScope()
    {
        ...
        var action = new JsonObject { ["role"] = "runtime_action", ["kind"] = "resource_lifecycle", ["action"] = new JsonObject { ["start"] = "b0", ["end"] = "b5" },
            ["execution"] = "generated_workflow", ["boundary"] = null, ["resource"] = new JsonObject { ["start"] = "b3", ["end"] = "b5" }, ["necessity"] = new JsonObject { ["state"] = "unspecified", ["evidence"] = null },
