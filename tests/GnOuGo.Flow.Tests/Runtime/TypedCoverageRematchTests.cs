...
{
    private static JsonObject Invoke(string name, params object[] args) => (JsonObject)typeof(WorkflowPlanExecutor)
    ...
    {
        JsonObject Match(string id) => new() { ["status"] = "matched", ["reason"] = "Declared producer", ["catalog_ids"] = new JsonArray(id),
            ["candidate_catalog_ids"] = new JsonArray(), ["decision_operation_id"] = "", ["conditional_mode"] = "" };
        var original = new JsonObject { ["operation_matches"] = new JsonObject { [affected] = Match("old"), [retained] = Match("unchanged") },
            ["constraint_matches"] = new JsonObject { ["policy"] = new JsonObject { ["status"] = "policy_only", ["denied_catalog_ids"] = new JsonArray(),
                ["candidate_catalog_ids"] = new JsonArray(), ["reason"] = "Retained locked policy" } } };
