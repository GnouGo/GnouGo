...
class DemoMcpSession:
    ...
    async def list_resources_async(self) -> list[McpResourceInfo]:
        return [McpResourceInfo(uri="demo://docs", name="docs", description="Demo docs", mime_type="text/plain")]
    ...
    async def get_prompt_async(self, prompt_name: str, arguments: Any):
        return McpGetPromptResult(
            ...
            messages=[McpPromptMessage(role="assistant", content=f"prompt:{prompt_name}:{arguments}")],
        )
...
class DemoMcpFactory:
    def __init__(self) -> None:
        self.server_metadata = [
            McpServerMetadata(
                name="demo",
                description="Demo MCP server exposing search/summarize capabilities for local CLI runs.",
            )
        ]
