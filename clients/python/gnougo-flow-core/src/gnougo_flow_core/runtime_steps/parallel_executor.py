...
"""
    documented_exceptions = [
        ...
        (ErrorCodes.PARALLEL_LIMIT, False, "parallel branches exceed configured limit."),
    ]

    async def execute_async(self, ctx: StepExecutionContext) -> Any:
        if not ctx.step.branches:
            raise WorkflowRuntimeException(ErrorCodes.INPUT_VALIDATION, "parallel step requires 'branches'")
        if len(ctx.step.branches) > ctx.limits.max_parallel_branches:
            raise WorkflowRuntimeException(ErrorCodes.PARALLEL_LIMIT, "Parallel branches exceed limit")
        ...
        async def run_branch(branch_steps: list[CompiledStep], branch_index: int) -> tuple[int, dict[str, Any]]:
            if sem:
                await sem.acquire()
            try:
                branch_data = {"inputs": dict(ctx.data.get("inputs", {})), "steps": dict(ctx.data.get("steps", {})), "env": dict(ctx.data.get("env", {}))}
                branch_result = RunResult(success=True)
                await ctx.engine.execute_steps_async(branch_steps, branch_data, branch_result, ctx.limits, ctx.call_depth, set(ctx.call_stack), ctx.telemetry_span)
                return branch_index, branch_data
            finally:
                if sem:
                    sem.release()

        tasks = [run_branch(branch, i) for i, branch in enumerate(ctx.step.branches)]
        results = await asyncio.gather(*tasks)
        ordered = [item for _, item in sorted(results, key=lambda x: x[0])]
        return {"branches": [item.get("steps", {}) for item in ordered]}
