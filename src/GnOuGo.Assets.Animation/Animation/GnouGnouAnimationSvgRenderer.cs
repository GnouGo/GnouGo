...
public static class GnouGnouAnimationSvgRenderer
{
        ...
        GnouGnouAnimationRenderOptions? options = null)
    {
        ...
        var description = Escape(options.Description ??
            $"Synthetic workflow preview on a clean white canvas with {plan.Actors.Count} GnOuGo actors.");
  ...
  <defs>
    <filter id="scene-shadow" x="-30%" y="-30%" width="160%" height="160%">
      <feDropShadow dx="0" dy="4" stdDeviation="5" flood-color="#17324d" flood-opacity=".14"/>
    </filter>
  ...
  <style>
    ...
    .route-outline, .route-surface, .route-centerline { fill: none; stroke-linecap: round; stroke-linejoin: round; }
    .route-outline { stroke: #cbd5dc; stroke-width: 46; }
    .route-surface { stroke: #53616d; stroke-width: 38; transition: stroke .2s ease; }
    .route-centerline { stroke: #fff; stroke-width: 3.5; stroke-dasharray: 13 15; }
    .flow-edge.is-active .route-outline { stroke: #a9c9e9; }
    .flow-edge.is-active .route-surface { stroke: #3f7fb5; }
    .flow-edge.is-active .route-centerline { stroke: #eaffff; }
    .flow-edge.is-success .route-surface { stroke: #458467; }
    .flow-edge.is-failed .route-surface { stroke: #b84f59; }
    .flow-node[data-selected="false"] { opacity: .4; }
    .control-marker { fill: #fff; stroke: #53616d; stroke-width: 10; }
    .control-marker-ring { fill: none; stroke: #fff; stroke-width: 2.5; stroke-dasharray: 8 9; }
    .control-glyph { font-size: 30px; font-weight: 950; text-anchor: middle; dominant-baseline: middle; fill: #315f82; }
    .flow-node.is-active .control-marker { stroke: #3f7fb5; }
    .flow-node.is-success .control-marker { stroke: #458467; }
    .flow-node.is-failed .control-marker { stroke: #b84f59; }
...
""");
    ...
    private static void AppendFlowNodes(StringBuilder builder, IReadOnlyList<AnimationFlowNode> nodes)
    {
                         ...
                         and not AnimationFlowNodeKind.Delivery))
        {
            ...
            builder.AppendLine("      <g class=\"control-node\" transform=\"translate(0 108)\">");
            builder.AppendLine("        <circle class=\"control-marker\" r=\"38\"/>");
            builder.AppendLine("        <circle class=\"control-marker-ring\" r=\"29\"/>");
            builder.Append("        <text class=\"control-glyph\" x=\"0\" y=\"2\">").Append(glyph).AppendLine("</text>");
            builder.AppendLine("      </g>");
            builder.Append("      <text class=\"node-label\" x=\"0\" y=\"164\">").Append(Escape(node.Label)).AppendLine("</text>");
            builder.AppendLine("    </g>");
        }
        builder.AppendLine("  </g>");
    }
        ...
        IReadOnlyList<AnimationFlowNode> nodes)
    {
        ...
        builder.AppendLine("  <g id=\"workstations\" aria-label=\"Workflow roundabouts\">");
        foreach (var station in stations)
        {
            ...
            builder.Append("    <g id=\"").Append(station.Id).Append("\" class=\"workflow-station\" data-node-kind=\"")
                ...
                .Append(Number(station.Position.Y)).AppendLine(")\">");
            builder.AppendLine("      <g class=\"workflow-roundabout\">");
            builder.AppendLine("        <circle class=\"roundabout-outline\" r=\"80\"/>");
            builder.AppendLine("        <circle class=\"roundabout-road\" r=\"61\"/>");
            builder.AppendLine("        <circle class=\"roundabout-marking\" r=\"61\"/>");
            builder.AppendLine("        <circle class=\"roundabout-island\" r=\"35\"/>");
            builder.Append("        <text class=\"roundabout-glyph\" x=\"0\" y=\"2\">")
                .Append(StationGlyph(station)).AppendLine("</text>");
            builder.AppendLine("      </g>");
            builder.Append("      <text class=\"station-label\" x=\"0\" y=\"111\">")
                .Append(Escape(station.Label)).AppendLine("</text>");
            if (!string.IsNullOrWhiteSpace(station.StepType))
            {
                builder.Append("      <text class=\"node-label\" x=\"0\" y=\"132\" opacity=\".62\">")
                    .Append(Escape(station.StepType)).AppendLine("</text>");
            }
            builder.AppendLine("    </g>");
        }
        builder.AppendLine("  </g>");
