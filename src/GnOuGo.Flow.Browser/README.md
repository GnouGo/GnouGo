...
`GnOuGo.Flow.Browser` is a **stdio** MCP server based on **Playwright.NET**. It exposes a set of web navigation tools usable from `GnOuGo.Flow` or any compatible MCP client.
...
- open a page and directly read its HTML → `browser_get_content(url: "https://example.com", selector: body, format: html)`
- read a confirmation message after a submit → `browser_get_content(format: text)`
- extract links from a menu → `browser_get_content(selector: nav, format: html)`
- choose a cookie banner button → `browser_get_content(selector: body, format: html)` then decide between `browser_click_text` and `browser_click`
- identify the right field before filling a form → `browser_get_content(selector: form, format: html)` then `browser_fill`
- wait for a results block to appear or an overlay to disappear → inspect `html`, choose a stable selector, then `browser_wait`
- send `Enter` in a known field → `browser_press(selector: "input[name='q']", key: "Enter")`
- select an option in a real `<select>` → inspect `html`, retrieve the `value`, then call `browser_select`
...
```powershell
dotnet build .\src\GnOuGo.Flow.Browser\GnOuGo.Flow.Browser.csproj
powershell -ExecutionPolicy Bypass -File .\src\GnOuGo.Flow.Browser\bin\Debug\net10.0\playwright.ps1 install chromium
dotnet run --project .\src\GnOuGo.Flow.Browser\GnOuGo.Flow.Browser.csproj
```
...
- if a debugger is attached and no `Browser:*` values are provided, `GnOuGo.Flow.Browser` automatically applies these defaults: `Headless=false`, `SlowMoMs=250`, `HoldOpenMs=15000`, `KeepBrowserOpen=true`
...
`GnOuGo.Flow.Cli` and `GnOuGo.Flow.Server` are now configured with a `GnOuGo.Flow.Browser` MCP entry using `stdio` transport.
    ...
    "McpServers": {
      "GnOuGo.Flow.Browser": {
        ...
        "Args": [
          "run",
          "--project",
          "src/GnOuGo.Flow.Browser/GnOuGo.Flow.Browser.csproj"
        ]
      }
    }
  }
...
- The command `dotnet run --project src/GnOuGo.Flow.Browser/GnOuGo.Flow.Browser.csproj` assumes launch from the repository root in development.
