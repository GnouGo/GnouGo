...
For every provider type, `Metadata.Capabilities.SupportsTemperature: false` or `"temperature"` in `UnsupportedRequestParameters` causes the proxy to omit temperature before forwarding. Listing `"top_p"` there similarly omits top-p. These are optional sampling hints that clients can send automatically; models that reject them use their own defaults. Supported or undeclared hints are preserved. Malformed values still fail validation, and other unsupported fields still return errors. The traffic inspector retains the original client payload and shows the upstream payload with the unsupported sampling hints omitted.

The native Anthropic adapter explicitly sends `thinking: {"type": "disabled"}`. Some models now enable thinking by default, but this text/tool integration cannot preserve their signed thinking blocks through subsequent Chat Completions tool-result turns. Disabling thinking avoids interrupted streams without discarding those required blocks. Models that require thinking are outside this adapter's supported capabilities. Set `SupportsReasoningEffort: false` and `SupportsVision: false` for this integration, and `SupportsTemperature: false` plus `UnsupportedRequestParameters: ["temperature", "top_p", "top_k"]` for models that reject non-default sampling (such as Sonnet 5 and Opus 5). The Chat Completions interface does not accept `top_k`; it remains an explicit error if sent. See the [Anthropic default-thinking behavior](https://platform.claude.com/docs/en/models/sonnet-5/whats-new-sonnet-5#adaptive-thinking-on-by-default).
...
![Estimated costs and the applied pricing tier](docs/screenshots/estimated-costs.png)

Prices are optional, manually configured **per million tokens**, in a three-letter uppercase currency code. The proxy never downloads prices, converts currencies, or chooses rates by model name. [The pricing example](examples/pricing.json) uses illustrative rates; replace them with your contracted prices. Omit pricing for an unpriced model. Explicit zero input/output prices mean free; missing base input/output prices or missing usage mean **Unknown**.

Set each model's `Metadata.Pricing` with `Currency`, `InputPer1MTokens`, `OutputPer1MTokens`, and optionally `CachedInputPer1MTokens`, `CacheWriteInputPer1MTokens`, and `ReasoningOutputPer1MTokens`. Optional category rates fall back to the input or output rate. Cache reads and writes are subtracted from total input; reasoning tokens are subtracted from total output before each category is charged. Anthropic cache counts are normalized into total input; repeated cumulative streaming usage replaces previous counts. All calculations use decimal arithmetic.

Optional model-level `PricingTiers` entries contain `InputTokensAbove` and a complete replacement `Pricing` object. The highest threshold **strictly below reported input tokens** applies to the whole request, including its output. Exactly-at-threshold requests retain the lower tier. Thresholds must be unique and nonnegative, rates nonnegative, and every tier must use the base currency. Changing prices requires a restart. Source, region, effective date, and retrieval date can be recorded as strings in `Metadata.Extra`; these notes do not affect calculation.

Traffic summaries/details expose `cost` with status (`estimated`, `partial`, or `unknown`), decimal amount, currency, category breakdown, and the applied `inputTokensAbove` (null for base pricing). Interrupted or active calls with reported usage have partial estimates; absent usage is never inferred from text. Snapshots include `costTotals` grouped by currency and `unknownCostCalls`. Dashboard totals cover **retained history**, including calls outside current filters. Clearing or evicting calls updates totals. These estimates are not billing records and exclude unreported usage, taxes, exchange rates, and contractual adjustments. All captures and totals remain in memory.
...
For OpenAI-compatible and Copilot models, set `Metadata.Capabilities.SupportsReasoningEffort: true` and `SupportedReasoningEfforts` to the exact levels accepted by your upstream. `/api/setup` exports them as the model's `supportsReasoningEffort` array with `reasoningEffortFormat: "chat-completions"`. Optional model-level `DefaultReasoningEffort` must belong to that list and is exported as `defaultReasoningEffort`. After updating `chatLanguageModels.json` in each profile you use, set `"chat.experimentalModelPicker": false` in that profile's settings and reload VS Code. Use the arrow beside the model in its picker to select **Thinking Effort**. The selected value is forwarded as `reasoning_effort`. The default level may appear without a suffix in the model label; this does not mean the choices are missing. Existing chat sessions remember their selected level. No levels are invented for missing metadata, models that reject `reasoning_effort`, or the native Anthropic/Ollama adapters, which do not implement reasoning translation. All configured models remain listed regardless of reasoning support.

Verify reasoning levels with function tools present, not just a plain chat prompt. Some gateways accept higher reasoning levels for text-only Chat Completions but require `reasoning_effort: "none"` when function tools are supplied; they require the Responses API to combine tools and reasoning. For those deployments, retain the supported plain-chat levels, set `DefaultReasoningEffort: "none"`, and select **None** for Agent tool sessions. This includes the [documented GPT-5.6 restriction](https://learn.microsoft.com/en-us/azure/foundry/foundry-models/concepts/models-sold-directly-by-azure#gpt-56). The proxy does not implement Responses or silently downgrade a selected effort. Requests outside explicitly configured levels fail locally with instructions to refresh the client configuration; unknown capability metadata remains transparent. Provider restrictions must be verified against the actual endpoint, even when its model catalog lists reasoning and tools separately.
...
Failed calls show the provider's captured JSON error message below the safe HTTP summary, using the same redacted traffic body as the raw inspector. This makes rejected parameter combinations visible without copying upstream bodies into exception messages, logs, or telemetry. Non-JSON or incomplete error bodies remain available in **Raw payloads**.
...
To verify reasoning controls using an existing profile's model settings, run the installed desktop application with an isolated copy. The source profile must use the standard picker. For a model configured with `none`, `low`, `medium`, `high`, and `xhigh`:

```bash
PLAYWRIGHT_MODULE_PATH=/tmp/gnougo-proxy-browser/node_modules/playwright/index.mjs \
PROXY_REASONING_PROFILE="$HOME/Library/Application Support/Code/User" \
PROXY_REASONING_MODEL_IDS="internal/code" \
  node scripts/smoke-proxy-copilot-reasoning.mjs
```

Use the corresponding `User/profiles/<profile-id>` directory to check another profile, and repeat for each model. This test copies the selected model entries and picker setting, without credentials or unrelated profile settings. It opens the real Thinking Effort control, checks all five choices, selects High, sends a real text-only request through VS Code, verifies the reported estimate and answer, then restores None in the test session. The test uses Ask mode's built-in `/explain` command on a small Python expression and verifies that no function tools are sent. In Copilot 0.67, ordinary Ask includes read-only tools and still injects `session_store_sql` even after all visible tools are deselected; disabling local indexing did not remove that injected tool in the tested version. Therefore, use **None** for ordinary Ask and Agent sessions against gateways that reject tools with higher reasoning. The selector remains available, and High works on the tested text-only Explain route. The smoke test does not change the normal profiles' tools or session-search settings. Screenshots and evidence stay in a private temporary directory. It never quits or restarts existing VS Code instances. A separate Agent smoke with None verifies tool execution; this does not add Responses support.
