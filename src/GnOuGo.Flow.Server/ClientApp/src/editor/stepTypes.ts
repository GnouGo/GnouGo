...
export const STEP_TYPES: StepTypeDef[] = [
  ...
  },
  {
    ...
    category: 'workflow',
    color: '#a855f7',
    description: 'Plan a typed workflow with behavior review, validation and final approval',
    fields: [
      { name: 'raw_prompt', label: 'Intent', type: 'string', required: true, placeholder: '${data.inputs.prompt}' },
      { name: 'generator', label: 'Model configuration (JSON)', type: 'json', required: true, placeholder: '{ "reasoning": "low", "max_input_tokens": 12000, "max_output_tokens": 8192 }' },
      { name: 'intent_clarification', label: 'Clarification (JSON)', type: 'json', placeholder: '{ "max_rounds": 3, "max_questions": 15 }' },
      { name: 'capability_preflight', label: 'Capability constraints (JSON)', type: 'json', placeholder: '{ "requirements": [], "constraints": [] }' },
      { name: 'policy', label: 'Policy (JSON)', type: 'json', placeholder: '{ "allowed_step_types": ["llm.call", "template.render"] }' },
