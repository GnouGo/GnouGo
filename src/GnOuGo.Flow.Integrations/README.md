...
The factory stores schema-8 sessions, reservations, receipts and cumulative budgets through public encrypted KeyVault record APIs in `flow-planning-*-v8` collections. Exclusive tenant/session leases live under `.GnOuGo/data/flow-planning-v8/leases`. Old formats are rejected. Resume the same run ID and request; completed calls replay without a new charge and uncertain dispatches stop without redispatch.
...
Planner decisions and their continuations use the existing encrypted workflow session journal and exclusive owner lease. Additive schema-8 request defaults are normalized when comparing stored definitions. An injected `IPlanningDecisionProvider` handles planner answers without changing runtime `human.input`. Run `dotnet test tests/GnOuGo.Flow.Integrations.Tests -m:1 -warnaserror` to verify journal replay and approval boundaries.

Scoped business-revision clarification uses the same separate planner provider as planner decisions. Its proposal and answer are encrypted alongside the accepted session, and the owning runtime checkpoints answers before acknowledgement. Restart restores the saved repair and cumulative counters without replaying the surrounding workflow. Legacy clarification and runtime `human.input` remain unchanged.
