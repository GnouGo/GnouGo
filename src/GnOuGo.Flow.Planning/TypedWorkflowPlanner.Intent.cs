...
{
    private sealed record IntentSource(string Id, string Kind, string Text, string? QuestionContext = null);
    ...
    private async Task AssessIntentAsync(PlanningSnapshot state, IPlanningRuntime runtime, CancellationToken ct)
    {
        ...
        var remaining = state.ClarificationForms >= (limits?["max_rounds"]?.GetValue<int>() ?? 3) ? 0 :
            Math.Max(0, Math.Min(limits?["max_questions_per_round"]?.GetValue<int>() ?? 5,
                (limits?["max_questions"]?.GetValue<int>() ?? 15) - state.ClarificationQuestions!.Value));
        var sourceJson = new JsonArray(sources.Select(s => (JsonNode)new JsonObject
        {
            ["sourceId"] = s.Id, ["kind"] = s.Kind, ["text"] = s.Text, ["questionContext"] = s.QuestionContext
        }).ToArray());
        ...
        for (var attempt = 0; attempt < 2; attempt++)
        {
            ...
            if (diagnostics.Count == 0)
            {
                ...
                if (attempt > 0) state.Events.Add(new("intent_repair_succeeded", PlanningPhase.Intent, _time.GetUtcNow()));
            ...
            if (attempt == 1)
            {
                ...
                state.Events.Add(new("intent_repair_exhausted", PlanningPhase.Intent, _time.GetUtcNow(), diagnostics.Count));
                return;
            }
            locks = CaptureIntentLocks(json, sources);
            state.Events.Add(new("intent_repair_started", PlanningPhase.Intent, _time.GetUtcNow(), diagnostics.Count));
    ...
    private static List<IntentSource> IntentSources(PlanningSnapshot state)
    {
        var sources = new List<IntentSource> { new("request", "user_request", state.Request.Prompt) };
        if (state.Request.Options["generator"]?["context"]?.GetValue<string>() is { Length: > 0 } context)
            sources.Add(new("host", "host_constraint", context));
        if (state.Request.ExistingYaml is { Length: > 0 } existing)
            sources.Add(new("existing", "existing_workflow", existing));
        for (var index = 0; index < state.Answers.Count; index++)
        {
    ...
    private static void ApplyIntent(PlanningSnapshot state, JsonObject json)
    {
        ...
        {
            Name = q!["id"]!.GetValue<string>(), Description = q["prompt"]!.GetValue<string>(), Type = "radio", Required = true, AllowCustomAnswer = true,
            ...
            {
                Value = o!["value"]!.GetValue<string>(), Description = o["description"]!.GetValue<string>(), Recommended = o["recommended"]!.GetValue<bool>()
            }).ToList()
        }).ToList();
    ...
    private static void InitializeClarificationUsage(PlanningSnapshot state)
    {
        state.ClarificationForms ??= state.Answers.Count + (state.Question is null ? 0 : 1);
        state.ClarificationQuestions ??= state.Answers.Sum(a => a.Answers.Count) + (state.Question?.Fields?.Count ?? 0);
        if (state.ClarificationForms < 0 || state.ClarificationQuestions < 0) throw new ArgumentException("Invalid clarification counters.");
    }

    private static void RecordClarification(PlanningSnapshot state, int questions)
    {
        state.ClarificationForms++;
        state.ClarificationQuestions += questions;
    }

    private static void ArchiveIntent(PlanningSnapshot state)
        => state.IntentHistory.Add(new(state.Revision, state.Request.Prompt, state.Answers.ToList(), state.Diagnostics.ToList()));
}
