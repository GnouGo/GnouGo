...
internal static class PlanningSourceDecisions
{
    ...
    internal static async Task InterpretAsync(PlanningSnapshot state, IPlanningRuntime runtime, CancellationToken ct)
    {
        ...
        var decisions = scopes.Select(scope =>
        {
            ...
            return new PlanningDecisionPages.Decision(DecisionId(scope.Reference, scope.Source.Text), PlanningHoleRequests.Object(("obligations", new JsonObject
            { ["type"] = "array", ["minItems"] = 0, ["maxItems"] = 4, ["items"] = item }),
                ("runtime", PlanningOperations.RuntimeSchema(state, scope.Source.Authority, scope.Boundaries.Schema))), new JsonObject
            {
        ...
        foreach (var scope in scopes)
        {
            ...
            runtimeEvidence.AddRange(PlanningOperations.ParseRuntime(state, scope.Reference, scope.Boundaries.Select, answer["runtime"]!.AsArray()));
    ...
    internal static bool IsOperation(PlanningObligation obligation) => PlanningSourceGroundingRules.OperationKinds.Contains(obligation.Kind, StringComparer.Ordinal) &&
        ...
        obligation.OperationAdmission is { Version: 2 } proof && proof.CanonicalId == obligation.Id;
