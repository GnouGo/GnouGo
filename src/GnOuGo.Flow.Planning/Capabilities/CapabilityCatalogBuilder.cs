...
internal static class CapabilityCatalogBuilder
{

    internal static CapabilityCatalog BuildSchemaAwareCapabilityCatalog(
        IReadOnlyList<McpServerDiscovery> discovered, IReadOnlySet<string> nativeStepTypes, IReadOnlyList<McpServerDiscovery>? completeDiscovery = null)
    {
    ...
    internal static bool IsWorkflowStructureNativeStepType(string stepType)
    ...
    internal static string FormatArtifactContractSummary(McpArtifactContract? contract)
    {
        if (contract == null)
            return string.Empty;
    ...
    internal static string FormatOutputContractSummary(McpToolInfo tool)
    {
    ...
    internal static string FormatCompositionContractSummary(McpCapabilityComposition? contract)
    {
        if (contract == null)
            return string.Empty;
    ...
    internal static string LimitCapabilityDescription(string? description)
    {
    ...
    internal static string BuildCompactOutputSummary(JsonNode? outputSchema)
    {
    ...
    internal static IReadOnlyList<CapabilitySchemaField> BuildCapabilitySchemaFields(
        JsonNode? schema, bool requiredOnly)
    {
    ...
    internal static void CollectCapabilitySchemaFields(
        JsonObject schema, string pointer, int depth, bool parentRequired, bool requiredOnly, List<CapabilitySchemaField> fields)
    {
    ...
    internal static string ReadCapabilitySchemaFieldDescription(JsonObject schema)
    ...
    internal static IReadOnlyList<string> ReadCapabilitySchemaStringValues(JsonObject schema)
    {
    ...
    internal static void CollectOutputSummaries(JsonObject schema, string pointer, int depth, List<string> values)
    {
    ...
    internal static void CollectArgumentSummaries(
        JsonObject schema, string pointer, int depth, IReadOnlyCollection<string> required, List<string> values, bool includeDescriptions)
    {
        if (depth >= CapabilitySchemaMaxDepth || schema["properties"] is not JsonObject properties)
            return;
        foreach (var property in properties.OrderBy(static item => item.Key, StringComparer.Ordinal))
    ...
    internal static string ReadCompactPropertyDescription(JsonObject schema)
    {
    ...
    internal static void CollectConditionalRequirementSummaries(
        JsonObject schema, string pointer, int depth, List<string> values)
    {
    ...
    internal static string ReadCompactSchemaType(JsonObject schema)
    {
    ...
    internal static IReadOnlyList<SelectorVariant> ExtractSelectorVariants(JsonNode? inputSchema)
    {
        if (inputSchema is not JsonObject root)
    ...
    internal static void CollectBoundedIndependentSelectorCombination(
        JsonObject root, List<SelectorVariant> variants)
    {
    ...
    internal static void CollectIndependentSelectorDimensions(
        JsonObject schema, string pointer, int depth, List<(string Path, IReadOnlyList<JsonNode?> Values)> dimensions)
    {
        if (depth >= CapabilitySchemaMaxDepth || schema["properties"] is not JsonObject properties)
            return;
        foreach (var property in properties.OrderBy(static item => item.Key, StringComparer.Ordinal))
    ...
    internal static void CollectSelectorVariants(
        JsonObject schema, string pointer, int depth, List<SelectorVariant> variants)
    {
        if (depth > CapabilitySchemaMaxDepth)
            return;
    ...
    internal static void CollectDiscriminatorVariants(
        JsonObject? discriminator, string pointer, List<SelectorVariant> variants)
    {
    ...
    internal static void CollectComposedBranchVariants(
        JsonArray? branches, string pointer, int depth, List<SelectorVariant> variants)
    {
        if (branches == null)
            return;
    ...
    internal static void CollectBranchSelectorDimensions(
        JsonObject schema, string pointer, int depth, List<(string Path, IReadOnlyList<JsonNode?> Values)> dimensions)
    {
        if (depth > CapabilitySchemaMaxDepth)
            return;
    ...
    internal static IReadOnlyList<JsonNode?> ReadDocumentedScalarValues(JsonObject schema, string path)
    {
    ...
    internal static bool AlternativeBindingsMatchSchema(CapabilityAlternative alternative, McpServerDiscovery server)
    {
    ...
    internal static bool IsJsonScalar(JsonNode? node) => node == null || node is JsonValue;
    ...
    internal static string CanonicalScalar(JsonNode? value) => value?.ToJsonString() ?? "null";

    internal static string CanonicalizeBindings(IReadOnlyList<CapabilityRequestBinding> bindings)
    ...
    internal static bool TryBuildConditionalBranchValues(
        IReadOnlyList<CapabilityCatalogEntry> entries, out IReadOnlyDictionary<string, string> branchValues)
    {
    ...
    internal static bool TryBuildConditionalActivation(
        IReadOnlyList<CapabilityCatalogEntry> entries, bool allowNoEffectOutcome, string requestedActivationMode, out IReadOnlyDictionary<string, string> branchValues, out string activationMode)
    {
        branchValues = new Dictionary<string, string>(StringComparer.Ordinal);
    ...
    internal static bool ConditionalCompositionInvocationsAreDistinct(
    ...
    internal static string FormatBindingsCompact(IReadOnlyList<CapabilityRequestBinding> bindings)
        => string.Join(",", bindings.OrderBy(static binding => binding.Path, StringComparer.Ordinal)
            .Select(static binding => binding.Path + "=" + CanonicalScalar(binding.Value)));

    internal static bool IsSensitiveSelectorPath(string path)
    {
    ...
    internal static JsonArray BuildRequestBindingsJson(IReadOnlyList<CapabilityRequestBinding> bindings)
    {
    ...
    internal static bool RequestContainsLiteralBindings(JsonNode? request, IReadOnlyList<CapabilityRequestBinding> bindings)
    {
        if (bindings.Count == 0)
            return true;
    ...
    internal static bool TryResolveJsonPointer(JsonNode root, string pointer, out JsonNode? value)
    {
        value = root;
    ...
    internal static bool IsDynamicExpressionNode(JsonNode? node)
