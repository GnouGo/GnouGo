...
Runtime actions pass [evidence-based admission](../../docs/planner-runtime-admission.md). Source interpretation independently distinguishes planning directives, public contracts, local executable behavior and requested external actions. Canonical declarations resolve first. Directives, declarations, omission defaults and policy subjects produce no operation question. Established runtime occurrences resolve deterministically; only ambiguous reuse of compatible occurrences uses bounded `intent_operations` pages. Local operations retain native nodes without external capability authority or capability-matching calls. Admission commits atomically before confirmation scoping and inventory. Neither proven local behavior nor an external action means `INTENT_OPERATION_UNRESOLVED`. Operation proof version 2 is additive within Schema-5; missing historical proof requires explicit reassessment.
