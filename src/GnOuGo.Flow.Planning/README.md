...
Effect grounding establishes realizations and selected operation identities before governing evidence is exposed. Governing domains contain only compatible selected realizations, never possible anchors. Same-clause or exact-baseline applicability can resolve a single target deterministically; other contributions use bounded realized-target decisions. Effect-proof version 4 binds these pages to the reconstructed realized set while retaining Schema-5. See [realization ordering](../../docs/planner-realized-governing.md).
