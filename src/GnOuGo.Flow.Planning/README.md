...
Direct, unshadowed and unreassigned `String`, `encodeURI`, `decodeURI`, `encodeURIComponent` and `decodeURIComponent` calls infer successful string results for one known JSON scalar argument, including nullable scalar unions. Opaque values, containers, aliases, constructors and unsupported calls remain uninferred. Conversion does not prove input presence or business validity; malformed URI escapes still fail at runtime. Binding and repair prompts share Core's computation inference profile; numeric conversions and unsupported control flow still require whole-result validation where consumers need stronger contracts.

Computation findings retain the expression, local origin, known contracts and originating producer location. Dependent failures point to their root producer while retaining their blocking status; independent errors remain visible. This context is additive in schema 8 and absent fields preserve legacy diagnostic serialization and artifact hashes. Technical inference failures never become planner decisions.

Replanning replaces a complete action or affected business scope atomically, preserves semantic requirements and invalidates downstream artifacts. Unchanged proposals stop early; unexpected host failures stop immediately. There are no executable holes, ranked catalog shortlists, local JSON patch batches or graph-to-intent reflection.
...
The shared response envelope permits business decisions during semantic planning, selection after complete catalog coverage, binding (including later batches), and scoped replanning. Every option passes the existing phase validators before exposure. Equivalent outcomes and single options continue deterministically. Technical repairs use the existing phase schema; host failures stop without questions. See [decision contracts and live validation](../../docs/planner-decisions-2026-09-23.md).

### Missing prerequisites and unavailable outcomes

Binding blockers may carry a prerequisite kind, affected output, selected consumer/contract path and root action. The host validates these references; independent blocking findings remain visible. Missing observations and artifact producers trigger technical, scoped semantic repair, never a business decision. Opaque observations still require whole-value runtime validation before field access.

Repairs checkpoint their candidate, exact target IDs and fingerprints before installation. Only affected actions and changed dependency boundaries are replaced. Identical siblings retain catalog coverage and selections; valid accepted binding prefixes survive. Coverage, reusable work and the known minimum calls are checked before the new semantic, grounding and binding state is installed together. An infeasible repair remains visible as unapplied, with the previous state intact. A saved checkpoint resumes without another proposal call or replenished counters.

An unavailable required business outcome cannot become an ordinary preferred option. Interactive planning proposes a concrete scoped revision through business clarification; explicit acceptance is required, and rejection stops planning. Auto stops with the unmet requirement. Switching modes never consents to reduced requirements. The eight-call/two-replan defaults are unchanged. Estimates are lower bounds: unknown selections, scenarios or further repairs can still exhaust the remaining allowance. See [prerequisite repair verification](../../docs/planner-prerequisite-repair-2026-09-23.md).
