...
[Canonical execution-request authority](../../docs/planner-execution-request-authority.md) establishes request proof 1 before property qualification. Eligible owned clauses are grouped by intersecting candidate-effect domains; one bounded cohort answer records positive requests and explicit non-request outcomes. Exact baseline nodes retain deterministic authority. Possible effects and preliminary runtime facts do not authorize performance. Support is projected from current request proofs; property schemas cannot select support, extend a predicate or change its effect. Nested property domains contain only references properly inside their fixed request parent. Contribution 7 and admission 17 bind this directional proof chain in Schema-5. Coverage, applicability, necessity, dependency authority and canonical IDs retain their existing responsibilities.
