...
Effect grounding establishes realizations and selected operation identities before governing evidence is exposed. Governing domains contain only compatible selected realizations, never possible anchors. Only exact current owner proof can resolve attachment deterministically; same-clause, same-parent, shared workflow and singleton target cardinality do not prove applicability. Effect-proof version 7 and applicability proof 2 bind these pages to the reconstructed realized set while retaining Schema-5. See [realization ordering](../../docs/planner-realized-governing.md).
...
[Exact fallback ownership](../../docs/planner-fallback-ownership.md) separates executable references from the governing source domain. Current request proofs fix executable support before the dependent joint clause/property decision. Every non-whitespace clause span must be accounted for by qualified units or exact canonical contract coverage. Additional clause evidence can qualify rules, conditions, runtime fallbacks or properties, but cannot extend executable support. Source-only properties carry current source/optional semantic-obligation bindings without a fabricated runtime parent, execution kind or necessity. Their applicability is established against realized effects before atomic admission. Existing obligation kinds and grounding remain validated; declaration defaults remain contract-owned.

[Canonical execution-request authority](../../docs/planner-execution-request-authority.md) establishes request proof 1 before property qualification. Eligible owned clauses are grouped by intersecting candidate-effect domains; one bounded cohort answer records positive requests and explicit non-request outcomes. Exact baseline nodes retain deterministic authority. Possible effects and preliminary runtime facts do not authorize performance. Support is projected from current request proofs; property schemas cannot select support, extend a predicate or change its effect. Nested property domains contain only references properly inside their fixed request parent. Contribution 8 and admission 18 bind this directional proof chain in Schema-5. Coverage, applicability, necessity, dependency authority and canonical IDs retain their existing responsibilities.

Required governing ownership derives residual lexical portions after exact request support and canonical declaration coverage. Every portion has a required bounded semantic answer; adjacent portions group explicitly while runtime/grounding boundaries remain separate. Group labels are response-local and never persistent identities. Existing semantic kinds filter exclusions and conflicting labels before dispatch. Nested properties remain inside fixed request parents. Contribution proof 8 and admission 18 bind the normalized result and full coverage validation; incomplete historical property answers cannot authorize current proofs. Schema-5 storage, request proof 1, applicability and call/token allowances remain unchanged.
