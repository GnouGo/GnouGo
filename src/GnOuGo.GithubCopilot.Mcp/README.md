...
The review session has no tools, no configuration discovery, no write permission, and is deleted after completion. Publication is deliberately outside this MCP and belongs to the Flow agent plus the official GitHub MCP. The publication gate fails closed for stale SHAs, invalid inputs, dry runs, and unapproved interactive runs. It derives `REQUEST_CHANGES` from blocking findings or established failed checks, `COMMENT` from incomplete verification, and `APPROVE` from a complete passing review, including reviews with zero findings. Every requested check has a named outcome and evidence; command checks retain original SDK execution observations and require observed completion, working directory, and exit codes. Assistant prose alone cannot establish command success. `auto_comment` remains restricted to `COMMENT`. The caller must display the concrete review before interactive confirmation and re-read the head immediately before publication. Merge is outside this contract.
