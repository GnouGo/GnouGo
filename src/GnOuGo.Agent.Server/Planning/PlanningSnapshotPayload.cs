...
{
    private const string Prefix = "gnougo-planning-br5:";

    internal static string Encode(PlanningSnapshot snapshot)
    {
        if (snapshot.SchemaVersion != 5) throw new InvalidOperationException("Unsupported planning snapshot schema.");
    ...
    internal static PlanningSnapshot Decode(string value)
    {
        try
        {
            ...
            return snapshot.SchemaVersion == 5 ? snapshot : throw new JsonException("Unsupported planning schema.");
        }
