...
{
    internal const string UnavailableMessage = "This saved planning session is incompatible with schema 9. Regenerate and approve the workflow. The original record is unchanged.";
...
internal static class PlanningSessionHistory
{
    internal static PlanningSessionInspection Read(string payload, string tenant, string id, bool workflow, DateTimeOffset updated, long? revision = null)
    {
        ...
        try
        {
            ...
            if (Number(root, "schemaVersion") != 9) return new(entry, null);
            ...
            if (state?.Request is null || state.Request.TenantId != tenant || state.Request.SessionId != id || state.SchemaVersion != 9 ||
