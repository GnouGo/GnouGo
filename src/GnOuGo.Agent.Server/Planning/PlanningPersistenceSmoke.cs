...
internal static class PlanningPersistenceSmoke
{
    ...
    public static async Task RunAsync(string directory)
    {
        ...
        { Disposition = "admitted", OperationAdmission = new(11, "action", "anchor", null,
            [new("decision", "clause", "anchor", "local_processing", PlanningOperationNecessity.Required, null, null) { RuntimeEvidenceId = "runtime", Disposition = "distinct", ResolutionOrigin = "deterministic", EffectId = "action",
                Effect = new(6, "effect-decision", "realizes", [new("main", "private_result", "result_realization", "private_result") { OccurrenceProof = new(1, "runtime", "main", null, new("invocation", "private_owner", "private_boundary"), null, "boundary-proof") }],
            ...
            { ExecutionContributions = [new(1, "runtime", "qualification", "qualified-domain",
                [new("qualified", "private_execution_evidence", "supports", "action", "requested_result_production", "private_result", "private_result", PlanningContributionOrigin.ModelQualification)], "qualified-proof")],
                RealizationCoverage = new(2, "coverage-decision", "coverage-domain", ["action"],
        ...
        if (prepared?.OperationAdmissionFingerprint != "operation-set-proof" || prepared.Obligations.Single().OperationAdmission is not { Version: 11 } admission ||
            admission.ExecutionContributions.SingleOrDefault() is not { Version: 1, ProofFingerprint: "qualified-proof" } contribution ||
            contribution.Contributions.Single().Basis != "requested_result_production" ||
            admission.RealizationCoverage is not { Version: 2, DomainFingerprint: "coverage-domain", ProofFingerprint: "coverage-proof" } coverage ||
            ...
            admission.Assignments[0].RuntimeEvidenceId != "runtime" || admission.Assignments[0].Effect is not { Version: 6 } effect ||
