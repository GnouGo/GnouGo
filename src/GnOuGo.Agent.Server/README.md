...
`/gnougo add` runs generic inventory-first capability preflight before workflow decomposition, using medium reasoning. The first structured call inventories runtime operations and constraints without seeing tools. A compact paged selector then chooses relevant physical MCP tools from one entry per tool, adds MCP-declared artifact producers and complete-operation wrappers, and expands authoritative schemas and selector variants only for that selected set before final matching. If inventory or required-candidate selection is incomplete, each stage gets one bounded repair. Complete discovery remains available to dry runs and deterministic validation, and the 256,000-character expanded-catalog guard is unchanged. Documented scalar selectors make logical variants of a multi-action MCP tool distinct and lock their literal request values through decomposition and YAML validation. Mutually exclusive runtime outcomes are locked as one exact-value switch rather than reported as creation-time ambiguity. Host configuration, internal provider/credential resolution, and the outer agent-persistence action are outside the generated workflow inventory. Required resource cleanup is generated under the Flow workflow-level `finally` array.
...
In inferred mode, matching is reported per operation as `matched`, `composed`, `conditional`, `local`, `ambiguous`, or `unavailable`. Operations explicitly classify their execution/effect kind and any runtime decision source; constraints explicitly classify exact denials versus structural policies, so Flow does not guess those semantics from GitHub-specific wording. One bounded repair can resolve malformed, unknown-ID, ambiguous, or initially unavailable decisions while preserving already valid matches. If only genuine user-intent ambiguity remains, `/gnougo add` presents one batched form with each unresolved issue plus questions about scope, runtime rules, external-effect boundaries, success criteria, and failure behavior, then reruns the complete inference sequence once. Missing input, timeout, malformed output, unavailable capabilities, or another unresolved result fails closed as `cannot_plan_safely` or `unsupported`, with an action to clarify/abandon or revise/configure capabilities. It never asks the user to choose an outcome that depends on a future review. Remaining failures include sanitized `matching_issues` with the operation, status, concise reason, and at most eight compact candidate cards; raw clarification answers, full schemas, prompts, repository content, credentials, and model reasoning are not included.
...
The harness submits the same short user intention for three independent generations and simulates the human for initial inputs, a possible capability-clarification form, generated-YAML approval, publication rejection, and the scoped confirmation for the disposable fixture. It validates every discovered MCP call and literal selector, executes a read-only review against the configured public acceptance PR while denying publication, and exercises the confirmed write path only against a disposable draft fixture. Unexpected human prompts fail the test. It restores the previous default-agent setting and removes the fixture PR/branch, generated agents, and isolated workspaces in `finally`.
