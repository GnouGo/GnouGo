...
`/gnougo add` can generate an automated PR-review agent when its description requests pull-request review. The generation policy requires an isolated `.GnOuGo/data/reviews/` clone, exact Git MCP revision comparison, one ephemeral Copilot review session, structured line validation/deduplication, a fresh head-SHA check, and the fail-closed publication gate. Publication defaults to interactive; `dry_run` never writes and explicit `auto_comment` can only submit a GitHub `COMMENT`. Generated agents never approve, merge, push, commit, checkout, modify the reviewed PR, or delete its branch.
