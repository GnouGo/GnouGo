...
`/gnougo add` runs generic two-pass capability preflight before workflow decomposition. The first structured call inventories runtime operations and constraints without seeing tools; the second matches them to opaque IDs from a compact schema-aware catalog. Documented scalar selectors make logical variants of a multi-action MCP tool distinct and lock their literal request values through decomposition and YAML validation. Host configuration, internal provider/credential resolution, and the outer agent-persistence action are outside the generated workflow inventory. The command fails before persistence when a required operation is unavailable or uncertain. Required resource cleanup is generated under the Flow workflow-level `finally` array.
