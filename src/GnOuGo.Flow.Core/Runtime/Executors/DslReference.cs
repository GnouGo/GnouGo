...
Every custom function generated by `workflow.plan` must have JSDoc immediately before the declaration, with typed `@param` tags for every parameter and a typed `@returns` tag for the output:
```yaml
functions: |
  /**
   * Classifies a message by urgency.
   *
   * @param {string} text - Message text to classify.
   * @returns {string} Routing label: "urgent" or "normal".
   */
