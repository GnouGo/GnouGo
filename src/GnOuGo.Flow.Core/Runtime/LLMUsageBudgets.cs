...
public sealed record LLMUsageBudgetLimits
{
    ...
    public void Validate()
    {
        ...
        if (MaxCalls is null && MaxTotalTokens is null && MaxElapsed is null && MaxEstimatedCostUsd is null)
...
public sealed class LLMUsageBudgetScope
{
        ...
        TimeProvider? timeProvider = null)
    {
        ...
        _unaccountedCostGate = limits.MaxEstimatedCostUsd.HasValue ? new SemaphoreSlim(1, 1) : null;

        var now = _timeProvider.GetUtcNow();
        _snapshot = initialSnapshot is null
            ? new LLMUsageBudgetSnapshot { StartedAtUtc = now }
            : ValidateInitialSnapshot(initialSnapshot, now);
    }
    ...
    public LLMUsageBudgetScope CreateChild(
        LLMUsageBudgetLimits limits,
        ILLMUsageBudgetSink? sink = null)
        => new(limits, parent: this, sink: sink, timeProvider: _timeProvider);
        ...
        CancellationToken ct)
    {
        ...
        try
        {
            ...
            {
                snapshot = _snapshot;
                EnsurePreCallLimits(snapshot, request, costEstimator, stage);
        ...
        string stage)
    {
        ...
        if (Limits.MaxEstimatedCostUsd is { } maxCost)
        {
            if (snapshot.EstimatedCostUsd >= maxCost)
                throw CreateExceeded(stage, "estimated_cost_usd", maxCost, snapshot.EstimatedCostUsd, snapshot);

            decimal? priceProbe;
            try
            {
                priceProbe = costEstimator?.EstimateCost(request.Model, 1, 1, request.Provider);
            }
            catch
            {
                throw CreateUnverifiable(stage, "pricing", snapshot);
            }
            if (!priceProbe.HasValue)
                throw CreateUnverifiable(stage, "pricing", snapshot);
        }
    }
    ...
    {
        _ = reservation;
        var requiresTokens = Limits.MaxTotalTokens.HasValue || Limits.MaxEstimatedCostUsd.HasValue;
        ...
        if (Limits.MaxEstimatedCostUsd.HasValue)
        {
            ...
            try
            {
                estimatedCost = costEstimator?.EstimateCost(
                    request.Model,
        ...
        try
        {
            lock (_gate)
            {
                ...
                snapshot = current with
                {
                    ...
                    EstimatedCostUsd = checked(current.EstimatedCostUsd + (estimatedCost ?? 0m))
                };
                _snapshot = snapshot;
            }
        }
        ...
        if (Limits.MaxTotalTokens is { } maxTokens && snapshot.TotalTokens > maxTokens)
            throw CreateExceeded(stage, "total_tokens", maxTokens, snapshot.TotalTokens, snapshot);
        if (Limits.MaxEstimatedCostUsd is { } maxCost && snapshot.EstimatedCostUsd > maxCost)
            throw CreateExceeded(stage, "estimated_cost_usd", maxCost, snapshot.EstimatedCostUsd, snapshot);
    }
        ...
        DateTimeOffset now)
    {
        ...
        if (snapshot.StartedAtUtc == default || snapshot.StartedAtUtc > now
            ...
            || snapshot.TotalTokens < componentTotal || snapshot.EstimatedCostUsd < 0)
