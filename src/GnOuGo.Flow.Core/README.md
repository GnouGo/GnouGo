...
Each generation phase keeps one best candidate. A repair may replace it only by reaching a later validation checkpoint or strictly reducing the blocking diagnostic set. Stale fingerprints, unchanged candidates, and regressions are rejected atomically; two non-improving responses terminate with `WORKFLOW_PLAN_REPAIR_STALLED`. A new contract epoch is opened only when the immutable leaf/main contract fingerprint changes. Telemetry records only response mode, schema version, phase/attempt, fingerprints, epoch, progress, and stall reason—not prompts, responses, YAML, credentials, or human answers.
