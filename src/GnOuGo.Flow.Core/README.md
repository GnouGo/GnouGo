...
Execution-contribution proof v1 records owned subspans, positive support bases, effect/boundary bindings, origins and decision fingerprints. Admission v11 carries these proofs; coverage v2 may only use their qualified support/governing roles. Historical runtime `EvidenceRole` values remain deserializable but confer no current authority. Runtime proof v7 and effect proof v6 retain Schema-5 storage and unchanged operation identities.
