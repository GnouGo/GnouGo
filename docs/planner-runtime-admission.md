...
Operation proof version **2** binds runtime evidence and canonical declaration coverage. Adding derived admitted obligations no longer invalidates declaration proofs. Source proof **4**, declaration proof **5**, snapshot schema **5**, encrypted namespaces, receipts and cumulative budgets remain unchanged. Historical admission proofs require explicit reassessment; archived sessions are never upgraded or resumed automatically.
