...
Each capability retains its locked upstream operation dependencies. Executable-unit checks reject unrelated producer results before final assembly, including through aliases, loop bodies and accepted decision guards. Reducer repair reconstructs the typed condition slots and retains human-review context; repeated repair does not accumulate permission wrappers. Predicates must produce booleans; statically evident outcome-label returns are diagnosed before compilation, and runtime permission wrappers reject other response types instead of silently treating them as a negative decision.

Artifact identity follows original declared producers through transparent aliases, native containers and explicit workflow arguments. Matching scalar types and model-produced copies do not establish identity. Availability checks still apply before selecting any binding. Each node receives only the bindings available in its own execution scope. For an MCP call with a continuing error fallback, the optional `envelope` result channel preserves the complete success or failure result. Raw `response` paths remain unavailable when not guaranteed. Loop result contracts preserve these alternatives, so downstream steps can inspect or serialize failure results without inventing successful responses. A failure fallback that loses a required original artifact is diagnosed at the producer, before its consumer is generated.
...
Nominal scenario inputs are separate encrypted validation fixtures. The planner constructs literal examples against the declared inputs, with a bounded repair and the existing context/output limits. A request and input-contract fingerprint permits reuse after restart. These examples never become YAML defaults or external arguments. This avoids asking executable repair to accept an invalid generic string placeholder as a URL. Cached fixtures and literal defaults are validated before reuse; a null value cannot satisfy a required input. Explicit nested-loop scenarios use item fixtures derived from declared producer schemas, while nominal execution and the exported artifact remain unchanged. Missing or invalid item contracts remain inconclusive. Synthetic path coverage still does not prove live external behavior.
...
Semantic findings select exact operation/field locations. A finding about iteration, ordering or other behavior structure returns through a revised behavior plan and a new exact approval. Accepted answers, capability preparation and cumulative usage remain intact. Focused behavior corrections patch named nodes instead of regenerating the entire description; a loop wrapper preserves the original operation. The revision source, attempted patch and diagnostics survive restart. Older snapshots can propose a validated projection of the retained graph for a new review. Technical coverage findings remain separate from user-intent evidence and are supplied to subsequent construction. This prevents a field repair from silently changing operation cardinality.
...
The authorized cumulative campaign ceilings are EUR 300, 3,000 calls and 20 million tokens. Ledger amendments preserve previous charges and unresolved reservations. The existing KeyVault configuration is authorized; this does not assert a provider-enforced spending cap. The harness enforces reservations and configured output ceilings before dispatch, with hidden transport retries disabled. Both Agent.Server routing adapters use the shared request/response mapper, preserving those transport flags and provider-neutral completion status. An output ceiling is reported as such, including a response that spends its allowance on reasoning and emits no JSON.
...
An explicitly declared local predicate retains its own native decision contract even when it consumes only one physical operation. A composed producer still counts as one declared operation. V2 never replaces that predicate with an upstream enum; empty or unresolved inputs remain invalid. V1 retains its existing decision-selection behavior.
