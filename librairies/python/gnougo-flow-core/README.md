...
One best candidate is retained per phase. A replacement must advance deterministic validation progress or strictly reduce the blocking diagnostic set. Stale, unchanged, and regressive responses are rejected atomically, and two non-improving responses stop with `WORKFLOW_PLAN_REPAIR_STALLED`. Sanitized telemetry contains only mode, schema version, phase/attempt, fingerprints, epoch, progress, and stall reason.
